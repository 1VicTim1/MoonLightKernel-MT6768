...
static struct proc_dir_entry *FTS_proc_tp_lockdown_info_entry;

#define FTS_TP_SELFTEST "tp_selftest"
static struct proc_dir_entry *FTS_proc_tp_selftest_entry;
...
static int fts_fw_write_start(const u8 *buf, u32 len, bool need_reset)
{
    ...
    sprintf(Tp_name,"HX,FT8720,FW:0x0%d",fwver);
...
static int fts_fw_resume(bool need_reset, enum FW_TYPE fw_type)
{
    ...
    if ((fw_type == FW_GESTURE) ||
        ((fw_type == FW_AUTO) && upg->ts_data->gesture_bmode && upg->ts_data->suspended)) {
